...
if (isset($_POST['submit'])) {
    ...
    $validTime = validateAppointmentTime($atime);

    if (!$validTime) {
        echo "<script>alert('Invalid appointment time. Please select a time between 7:30 AM and 10:00 PM.');</script>";
        echo "<script>window.location.href='your-form-page.php'</script>";
        exit();
    }

    // No need to manually generate AptNumber, let the database handle it
    $query = mysqli_query($con, "INSERT INTO tblappointment (Name, Email, PhoneNumber, AptDate, AptTime, Services) VALUES ('$name', '$email', '$phone', '$adate', '$atime', '$services')");

    if ($query) {
        // Retrieve the auto-incremented AptID
        $aptID = mysqli_insert_id($con);

        // You can generate AptNumber based on AptID if needed
        $aptnumber = 202300 + $aptID;
...
{
    $validTimes = array("07:30 AM", "08:00 AM", /* ... Add all valid times until 10:00 PM ... */ "09:30 PM", "10:00 PM");

    return in_array($time, $validTimes);
}
?>
<!DOCTYPE html>
<html lang="en">
