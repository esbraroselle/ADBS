...
if (strlen($_SESSION['bpmsaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            exit(); // Stop further execution if the email format is invalid
        }

        // Allow only @gmail.com addresses
        $allowedDomain = '@gmail.com';
        if (strpos($email, $allowedDomain) === false) {
            echo "<script>alert('Email addresses must be from $allowedDomain domain.');</script>";
            echo "<script>window.location.href = 'add-customer.php'</script>";
            exit(); // Stop further execution if the email domain is not @gmail.com
        }

        // Validate mobile number format (has 10 digits)
        if (!preg_match('/^\d{10}$/', $mobilenum)) {
            echo "<script>alert('Invalid mobile number format. Please enter a valid 10-digit number.');</script>";
            echo "<script>window.location.href = 'add-customer.php'</script>";
            exit(); // Stop further execution if the mobile number format is invalid
        }

        // Prepend country code to mobile number
        $mobilenum = '+63' . $mobilenum;

        $query = mysqli_query($con, "INSERT INTO tblcustomers (Name, Email, MobileNumber, Gender, Details) VALUES ('$name', '$email', '$mobilenum', '$gender', '$details')");
